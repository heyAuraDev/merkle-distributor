...
// Source: @openzeppelin/contracts/token/ERC721/IERC721.sol
...
// Source: @openzeppelin/contracts/token/ERC721/extensions/IERC721Metadata.sol
...
// Source: @openzeppelin/contracts/token/ERC721/IERC721Receiver.sol
...
// Source: @openzeppelin/contracts/interfaces/draft-IERC6093.sol
...
// Source: @openzeppelin/contracts/token/ERC721/utils/ERC721Utils.sol
...
// Source: @openzeppelin/contracts/utils/Context.sol
...
// Source: @openzeppelin/contracts/utils/Panic.sol
...
// Source: @openzeppelin/contracts/utils/math/Math.sol
...
// Source: @openzeppelin/contracts/utils/math/SignedMath.sol
...
// Source: @openzeppelin/contracts/utils/Bytes.sol
...
// Source: @openzeppelin/contracts/utils/Strings.sol
...
// Source: @openzeppelin/contracts/utils/introspection/ERC165.sol
...
// Source: @openzeppelin/contracts/token/ERC721/ERC721.sol
...
// Source: @openzeppelin/contracts/interfaces/IERC165.sol
...
// Source: @openzeppelin/contracts/interfaces/IERC721.sol
...
// Source: @openzeppelin/contracts/interfaces/IERC4906.sol
...
// Source: @openzeppelin/contracts/utils/cryptography/Hashes.sol
...
// Source: @openzeppelin/contracts/utils/cryptography/MerkleProof.sol
...
// Source: contracts/HeyAuraCreditPass.sol
...
/// @notice Transferable certificates in ten shared credit tiers. A USD face value is not a token balance or redemption function.
/// @dev The immutable deployer-admin may update metadata. No admin/root setter, burn, upgrade, or forced-transfer mechanism.
contract HeyAuraCreditPass is ERC721, IERC4906 {
    error InvalidRoot();
    ...
    event Claimed(address indexed account, uint256 indexed tokenId, uint256 indexed tierId, uint256 creditUsdCents);
    event AdminMinted(address indexed recipient, uint256 indexed tokenId, uint256 indexed tierId, uint256 creditUsdCents);
    event WhitelistAdded(address indexed account, uint256 indexed tierId, uint256 creditUsdCents);
    event MetadataBaseURIUpdated(string previousURI, string newURI);
    /// @notice Collection metadata changed; marketplace refresh signal defined by ERC-7572.
    ...
    /// @notice Tier granted after deployment; zero means no additional on-chain allocation.
    mapping(address => uint256) public additionalTier;
    mapping(uint256 => uint256) public tierOf;
    mapping(uint256 => uint256) public creditUsdCents;

    constructor(bytes32 root_, string memory metadataBaseURI_)
        ERC721("heyAura Season One Credit Pass", "AURAPASS")
    {
    ...
    /// @notice Mint the caller's assigned pass. Admin-added tiers take priority and accept an empty proof.
    function claim(bytes32[] calldata proof) external {
        if (claimed[msg.sender]) revert AlreadyClaimed();
        uint256 tierId = _resolveTier(msg.sender, proof);
        if (tierId == 0) revert InvalidProof();
        uint256 amountUsdCents = amountForTier(tierId);

        // Commit eligibility before the ERC-721 receiver callback; transfers never reset it.
        claimed[msg.sender] = true;
        uint256 tokenId = _mintPass(msg.sender, tierId, amountUsdCents);
        emit Claimed(msg.sender, tokenId, tierId, amountUsdCents);
    }

    /// @notice Grant one claim at a predefined tier without modifying the initial Merkle root.
    /// @dev Added tiers take priority over initial Merkle allocations. Additions cannot be changed or removed
    ///      and do not permit a second claim by the same wallet.
    function addToWhitelist(address account, uint256 tierId) external {
        if (msg.sender != admin) revert NotAdmin();
        if (account == address(0)) revert InvalidRecipient();
        if (claimed[account]) revert AlreadyClaimed();
        if (additionalTier[account] != 0) revert AlreadyAdded();
        uint256 amountUsdCents = amountForTier(tierId);
        additionalTier[account] = tierId;
        emit WhitelistAdded(account, tierId, amountUsdCents);
    }

    /// @notice Check an allocation and whether its wallet still has an unused claim. No transaction is required.
    /// @dev Admin-added tiers are checked first and ignore the proof. Otherwise, test the ten fixed tiers
    ///      against the supplied initial Merkle proof. The amount is always derived from the resolved tier.
    ///      This checks eligibility only; it does not execute an ERC-721 receiver callback or check gas funding.
    function canClaim(address account, bytes32[] calldata proof)
        external view returns (bool)
    {
        if (account == address(0) || claimed[account]) return false;
        return _resolveTier(account, proof) != 0;
    }

    /// @notice Issue an additional pass of a predefined tier, including to a wallet outside the whitelist.
    /// @dev Admin mints do not consume the recipient's whitelist claim. There is no admin mint cap.
    function adminMint(address recipient, uint256 tierId) external returns (uint256 tokenId) {
        if (msg.sender != admin) revert NotAdmin();
        uint256 amountUsdCents = amountForTier(tierId);
        tokenId = _mintPass(recipient, tierId, amountUsdCents);
        emit AdminMinted(recipient, tokenId, tierId, amountUsdCents);
    }

    /// @notice Change the collection metadata and the shared metadata folder for existing and future passes.
    /// @dev Use an IPFS folder URI ending in '/' with collection.json and tier files 1.json through 10.json.
    ///      This cannot change on-chain tiers, credit amounts, ownership, or claim eligibility.
    function setMetadataBaseURI(string calldata newURI) external {
        if (msg.sender != admin) revert NotAdmin();
        _validateMetadataURI(newURI);
        string memory previousURI = _metadataBaseURI;
        if (keccak256(bytes(previousURI)) == keccak256(bytes(newURI))) return;
        _metadataBaseURI = newURI;
        emit MetadataBaseURIUpdated(previousURI, newURI);
        // IDs are consecutive from 1 and cannot be burned; refresh only existing tokens.
    ...
    /// @notice Collection-wide name, logo, description, and website metadata for marketplaces.
    function contractURI() external view returns (string memory) {
        return string.concat(_metadataBaseURI, "collection.json");
    }
    ...
    function amountForTier(uint256 tierId) public pure returns (uint256) {
        if (tierId == 0 || tierId > TIER_COUNT) revert InvalidTierId();
        uint256[10] memory amounts = [uint256(50000), 40000, 30000, 20000, 10000, 9000, 8000, 7000, 6000, 5000];
        return amounts[tierId - 1];
    }

    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        _requireOwned(tokenId);
        return string.concat(_metadataBaseURI, Strings.toString(tierOf[tokenId]), ".json");
    }

    function _validateMetadataURI(string memory baseURI) private pure {
        bytes memory uri = bytes(baseURI);
        if (uri.length <= 8 || uri[7] == bytes1("/") || uri[uri.length - 1] != bytes1("/")) revert InvalidMetadataURI();
        bytes memory prefix = bytes("ipfs://");
        for (uint256 i; i < prefix.length; ++i) {
            if (uri[i] != prefix[i]) revert InvalidMetadataURI();
        }
    }

    function _resolveTier(address account, bytes32[] calldata proof) private view returns (uint256)
    {
        uint256 addedTier = additionalTier[account];
        if (addedTier != 0) return addedTier;

        // Preserve the existing chain-bound Merkle leaves and proofs. Only ten tiers are possible.
        for (uint256 tierId = 1; tierId <= TIER_COUNT; ++tierId) {
            bytes32 leaf = keccak256(bytes.concat(keccak256(abi.encode(
                block.chainid, account, tierId, amountForTier(tierId)
            ))));
            if (MerkleProof.verifyCalldata(proof, merkleRoot, leaf)) return tierId;
        }
        return 0;
    }

    function _mintPass(address recipient, uint256 tierId, uint256 amountUsdCents) private returns (uint256 tokenId) {
        // Assign unique IDs and metadata before any receiver callback. Failed mints roll back all writes.
        tokenId = ++totalMinted;
        tierOf[tokenId] = tierId;
        creditUsdCents[tokenId] = amountUsdCents;
